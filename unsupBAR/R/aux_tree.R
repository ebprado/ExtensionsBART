...
get_predictions = function(trees, single_tree = FALSE, n) {
  ...
  if(single_tree) {
    ...
    if(nrow(trees$tree_matrix) == 1) {
      predictions = rep(trees$tree_matrix[1, 'mu'], n)
    } else {
    ...
    predictions = get_predictions(trees[[1]], single_tree = TRUE)  +
      get_predictions(partial_trees,
